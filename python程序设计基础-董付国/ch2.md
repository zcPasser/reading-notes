...
- 逻辑运算符
  ...
  - and、or 不一定返回 Ture和false，而是**得到最后一个被计算的表达式的值**；而 not 返回 true和false。
- 矩阵乘法@
  - 联合 numpy扩展库。

## 2.4：常用内置函数

- 都封装在内置模块\__builtins__。
- 内置函数dir查看内置函数和内置对象。
- 类型转换
  - bin,oct,hex
  ...
  - ascii
- 函数式编程
  - map(fun, seq)：fun映射到iterable的每个元素上，原序列不改变。
  - filter(fun, seq)：返回seq中为true的元素序列。
  - reduce(fun(a, b, ...), seq)：seq中元素作为参数，不断迭代。
...
当把同一个值赋值给多个变量时，很多情况下只是简单让2个变量引用同一个值，而不是在内存中保存2份同样的值。

使用这样的内存管理模式，可以尽可能地节省内存的使用。

2.5、运算符%可以对浮点数进行求余操作。